...
import { ClientOptions } from 'testtesttest';
...
export { ClientOptions } from 'testtesttest';
...
export function initMcpServer(params: {
  ...
  const client = new ReadmeV2(params.clientOptions);
  const capabilities = {
    ...defaultClientCapabilities,
...
export function init(params: {
  ...
  const client =
    params.client || new ReadmeV2({ defaultHeaders: { 'X-Stainless-MCP': 'true' }, logger: logger });
