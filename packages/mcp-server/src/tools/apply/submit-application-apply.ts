...
export const handler = async (client: ReadmeV2, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await maybeFilter(jq_filter, await client.apply.submitApplication(body)));
};
